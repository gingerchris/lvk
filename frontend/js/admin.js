...
var updatePost = function() {
    ...
    console.log(postID), target.addClass("btn-danger");
    $.ajax(meta.base + "admin/blog/update/post/" + postID).done(function(result) {
        target.removeClass("btn-danger"), $("#blog-" + postID).replaceWith(result), setTimeout(function() {
    ...
    {
        var target = $(this);
        $.ajax(meta.base + "admin/blog/update/" + meta.category).done(function(result) {
            target.removeClass("btn-danger"), $("#blog-content").replaceWith(result);
        });
    }
};

$(function() {
    $(document).on("click", ".post-update", updatePost), $(document).on("click", ".blog-update", updateAll);
});